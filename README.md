...
## Ручной ввод сети (когда парсер не справился)

Экстрактор срабатывает не всегда: на планах и профилях этого проекта текст
переведён в кривые, отметки лежат в невычисленных полях AutoCAD, а координатной
сетки в PDF нет. Чтобы это не блокировало работу, сеть можно описать текстом:
...
участок УТ-1 -> Н1: нитка=supply ду=426 длина=38.1 материал=Сталь давление=1.6 температура=130
    изгиб: 116340.0 110428.0 28.70
```

```powershell
python scripts/export_sample_ifc.py --model network.txt --output out/manual.ifc
```

Формат построчный, ключи русские (английские тоже принимаются), десятичная
запятая допустима. На непонятной строке, неизвестном ключе или нечисловом
значении разбор останавливается и называет номер строки — файл, который правят
руками, обязан говорить, что не так, а не терять данные наполовину.
`pdf_to_ifc.manual_input.format_manual_text()` выгружает модель обратно в тот же
формат, поэтому путь двусторонний: распарсили что смогли → выгрузили → инженер
дописал → загрузили обратно.
