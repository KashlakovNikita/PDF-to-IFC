...
- `gost_designation` у NetworkEdge — обозначение трубы по ГОСТ (задача 30)
  наряду с material/insulation, а не вместо них;
...
class NetworkEdge:
    ...
    # Задача 30: точное обозначение трубы, как оно записано в спецификации и в
    # эталонной модели — "Ст 426х9,0/560 ППУ-ОЦ в изоляции по ГОСТ 30732-2020".
    # Живёт ПАРАЛЛЕЛЬНО с material/insulation, а не вместо них: material="Steel"
    # нужен для фильтров и логики, а обозначение — это то, что заказывают и по
    # чему принимают, включая толщину стенки, диаметр оболочки и номер ГОСТа,
    # которых в разобранном виде в модели нет. Разбирать обозначение обратно на
    # части здесь намеренно не пытаемся: в проектах оно пишется по-разному.
